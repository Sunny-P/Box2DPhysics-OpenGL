...
public:
	...
	Model(std::string path, Camera* camera, lightInfo _lighting){

		// EDIT
		//this->program = ShaderLoader::GetInstance()->CreateProgram("Model.vs", "Model.fs");
		
		this->program = shaderLoader.CreateProgram("Resources/Shaders/Model.vs", "Resources/Shaders/Model.fs");
	...
	void Render(glm::mat4 _model)
	{
		for (GLuint i = 0; i < this->meshes.size(); i++) {
			...
			this->meshes[i].Render(camera, program, lighting, _model);
		}
	}
	...
	void SetScale(glm::vec3 _scaleVec)
	{
		for (GLuint i = 0; i < this->meshes.size(); i++)
		{
			this->meshes[i].SetScale(_scaleVec);
		}
	}
